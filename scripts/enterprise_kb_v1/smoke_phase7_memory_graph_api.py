...
def test_endpoint_api_multi_turn():
    ...
    result = {
        "endpoint_registered": False, "testclient_available": False,
        "testclient_status_code": 0, "response_has_memory_trace": False,
        "multi_turn_api_memory_pass": False, "skipped": True, "skipped_reason": "",
    }
    # Static
    sp = Path(__file__).resolve().parent.parent.parent / "src" / "service" / "service.py"
    if sp.exists():
        result["endpoint_registered"] = f'@router.post("{ENDPOINT_PATH}")' in sp.read_text(encoding="utf-8")
    ...
    try:
        ...
        saved = {}
        for mod in ["agents", "agents.agents", "agents.github_mcp_agent",
                     "agents.github_mcp_agent.github_mcp_agent",
                     "langchain_mcp_adapters", "langchain_mcp_adapters.client",
                     "langchain_mcp_adapters.sessions", "mcp.client.streamable_http"]:
            if mod not in sys.modules:
                sys.modules[mod] = MagicMock(); saved[mod] = True
        try:
            from service.service import app
            client = TestClient(app)
            sid = "p7_api_test"
            r1 = client.post(ENDPOINT_PATH, json={"query": "internal_engineering_docs 是什么？", "corpus": "auto", "session_id": sid})
            result["turn1_status"] = r1.status_code
            r2 = client.post(ENDPOINT_PATH, json={"query": "它和 official_docs 有什么区别？", "corpus": "auto", "session_id": sid})
            ...
            for mod in saved: sys.modules.pop(mod, None)
    except ImportError as e: result["skipped_reason"] = f"ImportError: {e}"
    except Exception as e: result["skipped_reason"] = str(e)[:200]

    result["pass"] = (
        result["endpoint_registered"] and result.get("testclient_available", False)
        and result.get("turn1_status") == 200 and result.get("turn2_status") == 200
        ...
        and result.get("graph_debug_memory_write", False)
...
def test_no_session_auto_id_multiturn():
    ...
    result = {
        ...
        "turn1_auto_id": "", "skipped": True, "skipped_reason": "",
    }
    try:
        from fastapi.testclient import TestClient
        from unittest.mock import MagicMock
        ...
        try:
            ...
            r1 = client.post(ENDPOINT_PATH, json={"query": "internal_engineering_docs 是什么？", "corpus": "auto"})
            ...
            r2 = client.post(ENDPOINT_PATH, json={"query": "它和 official_docs 有什么区别？", "corpus": "auto", "session_id": auto_sid})
            ...
            result["no_session_auto_id_multiturn_pass"] = (
                r1.status_code == 200 and auto_sid.startswith("auto_")
                and mt1.get("auto_generated_session_id") is True
                and r2.status_code == 200 and mt2.get("session_id") == auto_sid
                and mt2.get("recent_turns_count", 0) >= 1
                and mt2.get("rewrite_used_memory") is True
...
def test_direct_call_memory_trace():
    ...
    required = ["session_id", "memory_read_used", "recent_turns_count",
                "active_topic", "rewrite_used_memory", "memory_context",
                "memory_write_candidate", "memory_write_status",
                "candidate_only_not_committed", "auto_generated_session_id"]
    missing = [f for f in required if f not in mt]
    gd = resp.get("graph_debug", {})
    return {"fields_present": len(missing) == 0, "missing": missing,
...
def test_no_session_id_auto():
    ...
    mt = resp.get("memory_trace", {})
    return {"session_id": mt.get("session_id", ""), "is_auto": mt.get("session_id", "").startswith("auto_"),
            "auto_generated_session_id": mt.get("auto_generated_session_id", False),
            "pass": mt.get("session_id", "").startswith("auto_") and mt.get("auto_generated_session_id") is True}


def main():
    print("=== Phase 7 v1.3 Smoke: Memory Graph API ===")
    results = {"smoke": "phase7_memory_graph_api_v1.3", "timestamp": datetime.now(timezone.utc).isoformat()}

    api = test_endpoint_api_multi_turn()
    print(f"  api: endpoint={api['endpoint_registered']} status={api.get('testclient_status_code')} multi_turn={api['multi_turn_api_memory_pass']}")
    results["endpoint_api"] = api

    auto = test_no_session_auto_id_multiturn()
    print(f"  auto_id: pass={auto['no_session_auto_id_multiturn_pass']} propagated={auto['auto_session_id_propagated_to_state']} t1={auto.get('turn1_auto_id','')[:20]}")
    if auto["skipped"]: print(f"  auto_id skipped: {auto['skipped_reason'][:100]}")
