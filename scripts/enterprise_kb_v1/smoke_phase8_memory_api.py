...
def test_full_lifecycle():
    result = {
        ...
        "graph_create_candidate_ok": False, "has_pending": False,
        "approve_ok": False, "active_item_ok": False,
        "graph_reads_approved_memory": False, "disable_ok": False,
        "graph_excludes_disabled_memory": False, "events_ok": False,
        "skipped": True, "skipped_reason": "",
    }

    # Static check
    sp = Path(__file__).resolve().parent.parent.parent / "src" / "service" / "service.py"
    src = sp.read_text(encoding="utf-8")
    result["endpoint_registered"] = all(ep in src for ep in [
    ...
    try:
        ...
        try:
            ...
            rA = client.post(ENDPOINT, json={
                "query": "本项目必须使用 HPC 执行所有 retrieval eval",
                "corpus": "auto", "session_id": SID,
            })
            ...
            result["pending_not_used_for_rewrite"] = (
                not rewrite_A.get("long_term_memory_used_for_rewrite", False)
            )
            ...
            result["graph_uses_approved_memory_for_rewrite"] = (
                rewrite_E.get("long_term_memory_used_for_rewrite", False)
                and len(rewrite_E.get("approved_memory_ids_used", [])) >= 1
            )
            ...
            if memory_id:
                ...
                result["disable_ok"] = rF.status_code == 200 and rF.json().get("status") == "disabled"

            # G. Third query — disabled memory excluded
            rG = client.post(ENDPOINT, json={
                "query": "项目 embedding 策略",
                "corpus": "auto", "session_id": SID,
            })
            ltmG = rG.json().get("long_term_memory_trace", {})
            result["graph_excludes_disabled_memory"] = (
                rG.status_code == 200
                and (ltmG.get("approved_memory_count", 0) == 0
            ...
            result["disabled_memory_not_in_memory_context"] = (
                memory_id not in ltmG.get("approved_memory_ids", [])
            )
            result["disabled_memory_not_used_for_rewrite"] = (
                memory_id not in rewrite_G.get("approved_memory_ids_used", [])
            )
    ...
    result["pass"] = (
        result["endpoint_registered"] and result["testclient_available"]
        and result["graph_create_candidate_ok"] and result["has_pending"]
        and result["approve_ok"] and result["active_item_ok"]
        and result["graph_reads_approved_memory"] and result["disable_ok"]
        and result["graph_excludes_disabled_memory"] and result["events_ok"]
        ...
        and result.get("graph_uses_approved_memory_for_rewrite", False)
...
def main():
    ...
    results = {"smoke": "phase8_memory_api_v1.1", "timestamp": datetime.now(timezone.utc).isoformat()}
    api = test_full_lifecycle()
    print(f"  create_candidate={api['graph_create_candidate_ok']} pending={api['has_pending']} "
    ...
    if api["skipped"]: print(f"  SKIPPED: {api['skipped_reason'][:120]}")
