...
def classify_query(state: GraphState, llm: LLMClient | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "intent_trace": {"query": state.query, **result, "latency_ms": latency, "llm_mode": "mock" if llm.is_mock else "grounded_llm"},
    }
...
def _rule_based_classify(query: str) -> dict[str, Any]:
    ...
    ql = query.lower()
    internal_keywords = ["本项目", "phase", "hpc", "bge-m3", "评测", "语料", "registry", "代码摘要", "配置项", "失败案例"]
    official_keywords = ["fastapi", "chroma", "langgraph", "openai", "pydantic", "httpexception", "middleware"]
    ...
    if int_hits > 0 and off_hits > 0:
        return {"intent": "mixed", "confidence": 0.7, "reasoning": "rule: both internal and official keywords detected", "recommended_corpus": "dual"}
    if int_hits > 0:
        return {"intent": "project_inquiry", "confidence": 0.7, "reasoning": "rule: internal project keywords detected", "recommended_corpus": "internal_engineering_docs"}
    if off_hits > 0:
        return {"intent": "technical_reference", "confidence": 0.7, "reasoning": "rule: official docs keywords detected", "recommended_corpus": "official_docs"}
    return {"intent": "ambiguous", "confidence": 0.3, "reasoning": "rule: no strong signal", "recommended_corpus": "official_docs"}
