...
def _build_final_response_legacy(state: GraphState) -> dict[str, Any]:
    ...
    try:
        ...
        ltm_trace["long_term_memory_scope"] = ltm_trace.get(
            "long_term_memory_scope", "project:enterprise_kb_v1"
        )
        ltm_trace["pending_candidate_count"] = ltm_trace.get(
            "pending_candidate_count", len(ltm_svc.list_pending_candidates())
        )
    except Exception:
        pass
...
def build_final_response(state: GraphState) -> dict[str, Any]:
    """Assemble the project-scoped Phase 10 response."""
    from custom_graph.memory_runtime import build_final_response_v2

    return build_final_response_v2(state)
