...
def verify_evidence(state: GraphState) -> dict[str, Any]:
    ...
    retrieved_ids = {r.get("chunk_id", "") for r in (state.ranked_results or state.retrieval_results)}
    ...
    if not citations:
        ...
        return {
            ...
            "citation_trace": {
                ...
                "valid_citations": 0,
                "invalid_citations": 0,
                "violations": [{"reason": "no citations generated — evidence_verifier cannot validate empty citations"}],
                "all_citations_from_retrieved": False,
                "citation_validity": False,
                "latency_ms": latency,
            },
